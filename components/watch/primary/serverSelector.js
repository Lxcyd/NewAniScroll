    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex flex-wrap gap-2">
        {visible.map((server) => {
          ...
          // No availability dot and no red "degraded" styling anymore — only the
          // active server is highlighted (orange ring); every other available
          // server is a plain chip. Degraded (iframe-fallback) servers look
          ...
          return (
            <button
              ...
              className={`inline-flex items-center rounded-pill px-3 py-1.5 text-sm font-karla font-medium transition-all duration-200 ${baseClasses}`}
            >
              {server.name}
            </button>
          );
