...
# The hand-written map above covers the first audit lot only. Any lot built by
# the exporter carries its own slug per anime, so fold those in rather than
# rendering a wall of bare MAL ids — the hand-written label still wins where it
# exists, since it is the readable one.
for _lf in sorted(Path(__file__).resolve().parent.parent.glob("datasets/anime.*.json")):
    try:
        for _a in json.loads(_lf.read_text("utf-8")):
            NAMES.setdefault(int(_a["mal_id"]), str(_a.get("slug") or _a["mal_id"]))
    except Exception:
        continue
