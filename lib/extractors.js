...
async function tryVidmolyDomain(embedUrl, domain) {
  ...
  try {
    ...
    console.error(`[vidmoly] ${domain} fetch failed (${Date.now() - t0}ms): ${e.message}`);
    return null;
  }
  if (!res.ok) {
    console.error(`[vidmoly] ${domain} HTTP ${res.status} (${Date.now() - t0}ms)`);
    return null;
  }
  const html = await res.text();
  const hasM3u8 = html.includes(".m3u8");
  console.log(`[vidmoly] ${domain} ${html.length}b m3u8=${hasM3u8} (${Date.now() - t0}ms)`);
  ...
  try {
    ...
    for (const domain of VIDMOLY_DOMAINS) {
      try {
        ...
        if (hit) {
          console.log(`[vidmoly] HIT on ${domain} → ${hit.masterUrl.slice(0, 80)}… (total ${Date.now() - t0}ms)`);
          return {
            streams: [{
              url: hit.masterUrl,
              quality: "auto",
    ...
    console.error(`[vidmoly] EXHAUSTED all domains for ${embedUrl} (total ${Date.now() - t0}ms)`);
...
async function fetchSibnetEmbed(embedUrl) {
  ...
  // 3rd-tier fallback: Fly anime-proxy. Different IP pool again, sometimes
  // gets past Sibnet's per-IP block when both Vercel AWS and CF Worker pools
  // are throttled (verified for Death Note VO videoid 4670133, where direct
  // and worker both returned 403).
  if (ANIME_PROXY_URL) {
    const tp = Date.now();
    try {
      const res = await fetchThroughProxy(embedUrl, { headers: SIBNET_HEADERS }, 6000);
      if (res.ok) {
        const html = await res.text();
        if (looksGood(html)) {
          console.log(`[sibnet] OK fly-proxy ${embedUrl} (${Date.now() - tp}ms)`);
          return { html, via: "proxy" };
        }
      }
    } catch {}
    console.error(`[sibnet] fly-proxy failed for ${embedUrl} (${Date.now() - tp}ms)`);
  }
