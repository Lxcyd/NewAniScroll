...
export async function extractSibnet(embedUrl) {
  try {
    ...
    // TEMP diagnostic: if no player.src matched, log a snippet so we can see
    // what Vercel is actually receiving (anti-bot challenge? geo redirect?
    // empty body?). Remove once root cause is found.
    if (!m) {
      console.error(
        `[sibnet-debug] no source for ${embedUrl} | status=${res.status} | bytes=${html.length} | head=${html.slice(0, 300).replace(/\s+/g, " ")}`,
      );
      return { error: "sibnet: no source found" };
    }
    // TEMP: also log the extracted videoid so we can correlate input → output.
    console.error(
      `[sibnet-debug] ${embedUrl} → ${m[1]}`,
    );
