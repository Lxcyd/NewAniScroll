...
export async function extractSibnet(embedUrl) {
  ...
  try {
    ...
    /* UNE SECONDE CHANCE, avec un budget a elle.
       ...
       Un echec ici n'est donc pas la preuve que sibnet refuse : c'est souvent la
       preuve qu'on ne lui a pas laisse le temps de repondre. */
    if (!finalUrl) finalUrl = await resolveShard(3000);
