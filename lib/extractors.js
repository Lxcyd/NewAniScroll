...
export async function extractMovearnpre(embedUrl) {
  try {
    ...
    // Route through the Worker so EXTRACTION runs from the same Cloudflare
    // network the BROWSER uses for playback. The playback CDN (dramiyos-cdn,
    // acek-cdn, mindbodywellness.space) IP-binds its segment tokens, so
    // extracting from Vercel and playing back via Cloudflare used to break
    // mid-stream.
    const res = await fetchViaWorker(
      embedUrl,
    ...
    if (looksLikeMaster) {
      try {
        const masterRes = await fetchViaWorker(
          masterUrl,
