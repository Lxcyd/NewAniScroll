...
const SIBNET_SKIP_AFTER_REFUSALS = 2;
let sibnetShellRefusals = 0;
let sibnetShellBlocked = false;
...
async function fetchSibnetEmbed(embedUrl) {
  ...
  try {
    ...
    if (res.status === 403) {
      if (++sibnetShellRefusals >= SIBNET_SKIP_AFTER_REFUSALS && !sibnetShellBlocked) {
        sibnetShellBlocked = true;
        console.error("[sibnet] shell.php refuse 2 fois — on ira directement a la page de visionnage");
      }
    }
    if (res.ok) {
      const html = await res.text();
  ...
  if (WORKER_PROXY_URL && !sibnetShellBlocked) {
    const tw = Date.now();
    try {
  ...
  if (expectedId) {
    ...
    try {
      const res = await fetchWithTimeout(watchUrl, { headers: SIBNET_HEADERS }, 5000);
      if (noteSibnetStatus(res.status)) return null;
      if (res.ok) {
        const html = await res.text();
        if (looksGood(html)) {
 ...
 */
async function probeShard(url, headers) {
  try {
 ...
 */
async function retryOnFallbackShard(refusedUrl, refererUrl) {
  let target;
  try {
  ...
  for (const shard of SIBNET_FALLBACK_SHARDS) {
    ...
    const ok = await probeShard(candidate, {
      ...SIBNET_HEADERS,
      Referer: refererUrl,
      Range: "bytes=0-0",
    });
  ...
  try {
    ...
    if (!got) {
      ...
      return sibnetThrottled()
        ? { error: "sibnet: debit limite (429) — reessayer plus tard", transient: true, throttled: true }
        : { error: "sibnet: embed unreachable or decoy", transient: true };
    }
    ...
    try {
      ...
      if (chosen) {
        const direct = await probeShard(chosen, shardHeaders);
        if (direct) {
          finalUrl = direct;
        } else {
          /* The shard sibnet routed us to refuses this network. The signature
             ...
             signed. */
          const rescued = await retryOnFallbackShard(chosen, pageUrl || embedUrl);
