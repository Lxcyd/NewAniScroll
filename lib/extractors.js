...
export async function extractSibnet(embedUrl) {
  try {
    // Sibnet blocks Cloudflare Worker IPs at the source (returns 400 on the
    // first hop, no redirect). Vercel AWS IPs are accepted. We extract direct
    // and flag the stream to be played through the Vercel-side proxy too so
    // BOTH extraction and segment fetches share the AWS IP pool — the only
    // network sibnet doesn't filter against.
    ...
    return {
      streams: [{
        ...
        referer: "https://video.sibnet.ru/",
        useLocalProxy: true, // sibnet blocks CF Worker IPs — must play via Vercel
      }],
    };
  } catch (e) {
  ...
  try {
    ...
    return {
      streams: [{
        ...
        referer,
        useLocalProxy: true, // acek-cdn blocks CF Worker IPs — play via Vercel
      }],
    };
  } catch (e) {
...
export async function extractVoe(embedUrl) {
  try {
    ...
    // that's already invalid (token bound to a different IP, or the file was
    // removed mid-session). Without this check the player would loop on 403s
    // until our hls-error fallback kicks in 3-5s later. A quick HEAD probe
    // catches it now and lets the caller mark the server unavailable
    // immediately.
    try {
      const cdnHost = new URL(chosen).hostname;
      ...
      if (probe.status >= 400 && probe.status < 500 && probe.status !== 405) {
