...
export async function extractSendvid(embedUrl) {
  ...
  try {
    ...
    console.log(`[sendvid] OK ${embedUrl} → ${videoUrl.slice(0, 80)}… (${Date.now() - t0}ms)`);
    return {
      streams: [{
        url: videoUrl,
        quality: "default",
...
// ── Megaplay (megaplay.buzz/stream/mal/<malId>/<ep>/<sub|dub>) ────
// NOTE: the route is now keyed by MAL id. Megaplay retired the old
// /stream/ani/<aniListId>/<ep>/<sub> path (it times out); the current scheme
// is /stream/mal/<malId>/<ep>/<sub|dub>. The caller builds that URL.
...
// ── VidNest (new.vidnest.fun anime API → custom-base64 JSON) ──
// VidNest's anime player is itself an aggregator: its two servers (aniwave,
// anitaku) resolve an HLS master.m3u8 + multi-language subtitle tracks via the
// new.vidnest.fun API. The API wraps the JSON in { encrypted:true, data:"…" }
// where `data` is a custom-alphabet Base64 of the UTF-8 JSON — NOT real crypto
// (verbatim algorithm lifted from their client chunk's decryptCipherResponse).
// The CDN segments require Referer: https://megaplay.buzz/ (same as Megaplay).
const VIDNEST_API = "https://new.vidnest.fun";
const VIDNEST_REFERER = "https://megaplay.buzz/";
// Shuffled Base64 alphabet used by VidNest's decryptCipherResponse.
const VIDNEST_ALPHABET =
  "RB0fpH8ZEyVLkv7c2i6MAJ5u3IKFDxlS1NTsnGaqmXYdUrtzjwObCgQP94hoeW+/=";

function decodeVidnestPayload(data) {
  const map = {};
...
// VidNest's two anime backends. We try them in order and return the first that
// yields a playable HLS source. `sub` = "sub" | "dub".
function vidnestEndpoints(anilistId, episode, sub) {
  const s = sub === "dub" ? "dub" : "sub";
  return [
    { name: "aniwave", url: `${VIDNEST_API}/hianime/anime/${anilistId}/${episode}/${s}` },
    { name: "anitaku", url: `${VIDNEST_API}/anitaku/${anilistId}/${episode}/${s}/hd-2` },
  ];
}

async function fetchVidnestSource(endpoint) {
  const res = await fetchWithTimeout(
    endpoint.url,
    {
      headers: {
  ...
  if (!res.ok) return { error: `vidnest ${endpoint.name} HTTP ${res.status}` };
  let body;
  try {
    body = JSON.parse(await res.text());
  } catch (e) {
    return { error: `vidnest ${endpoint.name}: JSON parse (${e.message})` };
  }
  // The payload is wrapped only when `encrypted` is set; tolerate a plain JSON
  // response too in case a backend ever returns it unwrapped.
  if (body?.encrypted) {
    if (typeof body.data !== "string")
      return { error: `vidnest ${endpoint.name}: missing data field` };
    try {
      body = JSON.parse(decodeVidnestPayload(body.data));
    } catch (e) {
      return { error: `vidnest ${endpoint.name}: decode failed (${e.message})` };
    }
  }
  const m3u8 = (body?.sources || []).find((s) => s?.file)?.file;
  if (!m3u8) return { error: `vidnest ${endpoint.name}: no source in payload` };

  const subtitles = (body?.tracks || [])
    .filter((t) => t?.file && (t?.kind === "captions" || t?.kind === "subtitles"))
    .map((t) => ({
      file: t.file,
      label: t.label || "Subtitle",
      language: subtitleLabelToCode(t.label),
      kind: t.kind || "captions",
      default: !!t.default,
    }));

  // intro/outro skip ranges come free with the payload — surface them so the
  // player's skip overlay can use VidNest's timings when AniSkip has none.
  const skip = {};
  if (body?.intro?.end > 0) skip.intro = body.intro;
  if (body?.outro?.end > 0) skip.outro = body.outro;
  ...
  return {
    ...
    subtitles,
    ...(skip.intro || skip.outro ? { skip } : {}),
  };
}

export async function extractVidnest(anilistId, episode, sub) {
  let lastError = "vidnest: no source";
  for (const endpoint of vidnestEndpoints(anilistId, episode, sub)) {
    try {
      const result = await fetchVidnestSource(endpoint);
      if (result.streams?.length) return result;
      lastError = result.error || lastError;
    } catch (e) {
      lastError = `vidnest ${endpoint.name}: ${e.message}`;
    }
  }
