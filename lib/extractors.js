...
export async function extractVidmoly(embedUrl) {
  ...
  try {
    ...
    // and playback MUST share an IP.
    //
    // Tier 1: CF Worker (fetchViaWorker). When this succeeds, return the
    // raw masterUrl — the client's UniversalPlayer wraps it through
    // PROXY_BASE which is the same Worker, so segment fetches share IP.
    //
    // Tier 2: Fly anime-proxy (fetchThroughProxy). Worker IPs are often
    // 410-blocked by Vidmoly; Fly's pool is separate and frequently
    // passes. When Fly works, we MUST wrap the masterUrl through Fly
    // ourselves (so segments fetch from Fly's IP, not the worker's) and
    // return with directUrl:true so the client doesn't double-wrap.
    const tierWorker = async (domain) =>
      tryVidmolyDomain(embedUrl, domain, fetchViaWorker, "worker");
    const tierFly = async (domain) =>
      ANIME_PROXY_URL
        ? tryVidmolyDomain(embedUrl, domain, fetchThroughProxy, "fly")
        : null;
    ...
    for (const domain of VIDMOLY_DOMAINS) {
      const f = await tierFly(domain);
      if (f) {
        const wrapped = wrapWithAnimeProxy(f.masterUrl, "https://vidmoly.net/");
        if (wrapped) {
          console.log(`[vidmoly] HIT fly/${domain} → wrapped (total ${Date.now() - t0}ms)`);
          return {
            streams: [{
              url: wrapped,
              quality: "auto",
              isM3U8: f.masterUrl.includes(".m3u8"),
              referer: f.url,
              directUrl: true,
            }],
          };
