...
async function tryVidmolyDomain(embedUrl, domain) {
  ...
  // Always route extraction via the CF Worker — that's the same network
  // the client will use to fetch segments (NEXT_PUBLIC_PROXY_BASE is the
  // same worker), so the IP-bound master token stays valid for playback.
...
export async function extractVidmoly(embedUrl) {
  try {
    // Two key constraints:
    //   1. Extraction MUST share an IP with whoever fetches the segments —
    //      vidmoly's master m3u8 token is bound to the requester's IP. We
    //      route both via the CF Worker (fetchViaWorker), so the token is
    //      issued for a Worker IP, and the client then plays through the
    //      same Worker's m3u8 proxy.
    //   2. The m3u8 URL is returned RAW (no wrapping). The client's
    //      UniversalPlayer routes m3u8 streams through PROXY_BASE
    //      (NEXT_PUBLIC_PROXY_BASE = the same CF Worker), and the proxy
    //      rewrites the playlist's segment URIs to flow through itself —
    //      so every fetch on the playback path shares the Worker IP.
    for (const domain of VIDMOLY_DOMAINS) {
      try {
        ...
        // Hash fallback: vidmoly's HTML sometimes hides the source until a
        // second request with `?g=<hash>` is sent.
        const url = embedUrl.replace(/vidmoly\.(to|biz|net)/, domain);
        const res = await fetchViaWorker(
          url,
...
async function fetchSibnetEmbed(embedUrl) {
  // Try direct first — fastest path. If the response doesn't contain a
  // player.src OR the matched videoid disagrees with the URL we asked for,
  // fall back through the Cloudflare Worker (different IP pool, sometimes
  // gets past the bot block) and finally through the dedicated anime-proxy
  // if configured. Returns { html, via } where via is "direct"/"worker"/"proxy".
  ...
  const looksGood = (html) => {
    ...
    // The extracted path is /v/HASH/<videoid>.mp4 — the trailing id MUST
    // match the embed we asked for. If it doesn't, Sibnet served us a
    // decoy and we'd cache the wrong stream.
  ...
  // 1. Direct
  try {
    const res = await fetchWithTimeout(embedUrl, { headers: SIBNET_HEADERS }, 6000);
    if (res.ok) {
      const html = await res.text();
      if (looksGood(html)) return { html, via: "direct" };
      console.error(
        `[sibnet] direct mismatch for ${embedUrl} — falling through`,
      );
    } else {
      console.error(`[sibnet] direct HTTP ${res.status} for ${embedUrl}`);
    }
  } catch (e) {
    console.error(`[sibnet] direct fetch failed: ${e.message}`);
  }

  // 2. Cloudflare Worker
  if (WORKER_PROXY_URL) {
    try {
      const res = await fetchViaWorker(embedUrl, { headers: SIBNET_HEADERS }, 8000);
      if (res.ok) {
        const html = await res.text();
        if (looksGood(html)) return { html, via: "worker" };
      }
    } catch {}
  }

  // 3. Dedicated anime-proxy (Fly)
  if (ANIME_PROXY_URL) {
    try {
      ...
      if (res.ok) {
        const html = await res.text();
        if (looksGood(html)) return { html, via: "proxy" };
      }
    } catch {}
  }
...
export async function extractSibnet(embedUrl) {
  try {
    ...
    // Defence in depth: even after looksGood passed in fetchSibnetEmbed,
    // verify here once more — caller may have called extractSibnet without
    // going through fetchSibnetEmbed in the future.
    const expectedId = (embedUrl.match(/videoid=(\d+)/) || [])[1];
    const gotId = (initialUrl.match(/\/(\d+)\.mp4/) || [])[1];
    if (expectedId && gotId && expectedId !== gotId) {
      return {
        error: `sibnet: id mismatch (asked ${expectedId}, got ${gotId})`,
      };
    }
