...
export async function extractSendvid(embedUrl) {
  ...
  try {
    ...
    // Sendvid MUST be proxied — do NOT play it direct from the CDN.
    // The signed MP4 URL carries `ip=<extractor>` and the CDN binds the link to
    // THAT ip: a request from any other IP gets 403. Since the extractor runs on
    // the CF Worker and the VIEWER's browser has a different IP, a direct <video>
    // src would 403 for the user (verified: direct=403, via-proxy=206). An
    // earlier "direct-play probe" wrongly returned directUrl+noCors here — it
    // probed from a third IP that the CDN happened to tolerate, a false positive
    // — which BROKE Sendvid for real viewers (403 / "lent") AND killed the hover
    // preview (noCors taints the canvas). Routing through the Worker fixes both:
    // the Worker's IP matches the signed link (200/206), it adds CORS so the
    // preview works, and segments become edge-cacheable. Range works (206), so
    // the proxied preview only fetches the bytes around each thumbnail.
    console.log(
      `[sendvid] OK ${embedUrl} → ${videoUrl.slice(0, 80)}… (proxied, ${Date.now() - t0}ms)`,
    );
    return {
      streams: [{
        url: videoUrl,
