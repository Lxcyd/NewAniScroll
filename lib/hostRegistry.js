...
export const SERVER_TO_HOST = {
  ...
  // sendvid retire le 31/08/2026 (hote HS, cf. lib/servers.js) : sa disparition
  // d'ici retire `sendvid` de DISPLAYED_HOSTS, ce qui autorise l'importeur a
  // purger ses lignes — la table n'en portait aucune, rien n'est perdu.
  // Ansembed replaced anime-sama's Vidmoly entry (see lib/servers.js). It is
  // its OWN detector host, not an alias of `vidmoly-va`: voir-anime's vidmoly
  // is a different site with different uploads, hence a different encode —
  // measured on SnK ep1, ED at 23:55.2 here vs 24:05.7 there. Sharing rows
  // would serve one encode's OP/ED timing over the other's video.
  ...
  // Frembed's two chips play the SAME file — one master.m3u8 holding both the
  // `fr` and `ja` audio renditions (see lib/servers.js) — so unlike every other
  // pair here their OP/ED timings are identical by construction, and the
  // detector will fingerprint the same encode twice. That redundancy is
  // deliberate: the (host, lang) key is what the serve path translates an
  // active server id into, so collapsing both chips onto one lang row would
  // leave the other chip with no row to read.
  frembed: { host: "frembed", lang: "vf" },
  "frembed-vo": { host: "frembed", lang: "vostfr" },
};
...
/* …et dans l'AUTRE sens : une entree qui ne correspond a aucun serveur.
 *
 * Le garde ci-dessus n'a jamais rien vu venir de ce cote-la, et c'est comme ca
 * que `megaplay` est reste ici apres son retrait de lib/servers.js (08/09/2026)
 * — donc dans `DISPLAYED_HOSTS`, donc dans `host_versions.json` et dans le
 * `MULTI_HOSTS` du detecteur, trois listes tenues a la main qui se sont mises
 * d'accord sur un hote qui n'existe plus.
 *
 * Il AVERTIT au lieu de lever : une entree orpheline garde en vie les skips
 * OP/ED deja stockes pour cet hote (le purgeur de l'importeur s'appuie sur
 * `DISPLAYED_HOSTS`), et les jeter au premier import serait une perte de
 * donnees pour une question de menage. C'est donc un rappel, volontairement
 * bruyant, pas un couperet — megaplay est ici pour cette raison precise.
 */
const _orphelins = Object.keys(SERVER_TO_HOST).filter(
  (id) => !SERVERS.some((s) => s.id === id),
);
if (_orphelins.length) {
  console.warn(
    `[hostRegistry] entree(s) sans serveur correspondant : ${_orphelins.join(", ")} — ` +
      `elles gardent leurs skips OP/ED en base ; a retirer (ici, dans ` +
      `tools/opening-detector/host_versions.json et dans MULTI_HOSTS) le jour ` +
      `ou l'on accepte de purger ces lignes.`,
  );
}
