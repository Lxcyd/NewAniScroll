import { redis } from "@/lib/redis";
import { anilistFetch } from "./anilistFetch";

/**
 * Upcoming-anime carousel for the home page.
 *
 * Called from every home-page SSR. The underlying data (anime airing
 * "soon" in the current season) shifts at most once per day, so we
 * cache the resolved list in Redis for 6 hours.
 *
 * The AniList call itself goes through the global limiter, so even a
 * cold-cache stampede can't blow the rate budget.
 */

export const UPCOMING_CACHE_KEY = "anilist:upcoming:v1";
const CACHE_KEY = UPCOMING_CACHE_KEY;
const TTL_SECONDS = 6 * 60 * 60;

/* Written in place of the list when there is nothing to write.

   The homepage calls this on BOTH of its branches — Redis hit and Redis miss —
   so during the 02/09/2026 AniList outage every single visit spent a `redis.get`
   that could only miss, then an upstream round-trip that could only fail, and
   wrote nothing. Recording the absence turns that GET into a hit. Two minutes,
   not six hours: an empty carousel must not outlive the outage. */
const NONE_SENTINEL = "__none__";
const NONE_TTL_SECONDS = 120;

function currentSeasonYear() {
  const now = new Date();
  const m = now.getMonth();
  let season;
  if (m < 3) season = "WINTER";
  else if (m < 6) season = "SPRING";
  else if (m < 9) season = "SUMMER";
  else season = "FALL";
  return { season, year: now.getFullYear() };
}

const QUERY = `
  query ($season: MediaSeason, $year: Int, $format: MediaFormat) {
    Page(page: 1) {
      media(
        season: $season,
        seasonYear: $year,
        format: $format,
        isAdult: false,
        type: ANIME,
        sort: TITLE_ROMAJI,
      ) {
        id idMal
        title { romaji native english }
        startDate { year month day }
        status season format description bannerImage
        coverImage { extraLarge color }
        airingSchedule(notYetAired: true, perPage: 1) {
          nodes { episode airingAt }
        }
      }
    }
  }
`;

async function writeNone() {
  if (!redis) return;
  try {
    await redis.set(CACHE_KEY, NONE_SENTINEL, "EX", NONE_TTL_SECONDS);
  } catch {
    /* non-fatal */
  }
}

/**
 * @param {string | null | undefined} [prefetched] The raw Redis value when the
 *   caller already read CACHE_KEY in a batch (the home page folds it into its
 *   own MGET — one command instead of two per render). `undefined` means "not
 *   read": this function does its own GET, as before.
 */
const getUpcomingAnime = async (prefetched) => {
  if (redis) {
    try {
      const cached =
        prefetched === undefined ? await redis.get(CACHE_KEY) : prefetched;
      if (cached === NONE_SENTINEL) return null;
      if (cached) return JSON.parse(cached);
    } catch {
      /* fall through */
    }
  }

  const { season, year } = currentSeasonYear();
  const json = await anilistFetch({
    query: QUERY,
    variables: { season, year, format: "TV" },
    label: "upcoming",
    cacheSeconds: 0, // we handle our own longer cache below
  });
  if (!json?.data?.Page?.media) {
    await writeNone();
    return null;
  }

  const currentSeasonAnime = json.data.Page.media;
  let nextAiringAnime = currentSeasonAnime.filter(
    (anime) => anime.airingSchedule.nodes?.[0]?.episode === 1
  );

  if (nextAiringAnime.length >= 1) {
    nextAiringAnime.sort(
      (a, b) =>
        a.airingSchedule.nodes?.[0].airingAt -
        b.airingSchedule.nodes?.[0].airingAt
    );
  } else {
    nextAiringAnime = null;
  }

  if (redis) {
    try {
      if (nextAiringAnime) {
        await redis.set(CACHE_KEY, JSON.stringify(nextAiringAnime), "EX", TTL_SECONDS);
      } else {
        // A real "nothing airing first-episode soon" answer, not a failure —
        // but still worth recording so the next visit doesn't re-ask AniList.
        await redis.set(CACHE_KEY, NONE_SENTINEL, "EX", NONE_TTL_SECONDS);
      }
    } catch {
      /* non-fatal */
    }
  }

  return nextAiringAnime;
};

export default getUpcomingAnime;
