...
// A positive notice (e.g. "Progress Updated") should join the in-player toast
// stack when we're on the watch page, so it stacks with the player's other
// notices instead of floating as a separate sonner toast. When no player stack
// is mounted (any other page) it falls back to a normal sonner success toast.
const notifySuccess = (msg) => {
  if (typeof window !== "undefined") {
    // Dispatch synchronously; the in-player stack listener flips detail.consumed
    // to true if it handled it. If nothing consumed it (no player mounted), fall
    // back to a plain sonner toast so the notice still shows on other pages.
    const detail = { msg, consumed: false };
    window.dispatchEvent(new CustomEvent("aniscroll:playerNotice", { detail }));
    if (detail.consumed) return;
  }
...
`;
  ...
  `;
    ...
    if (media && media.type !== "MANGA") {
      ...
      notifySuccess(`Progress Updated: ${progress}`);
      // }
