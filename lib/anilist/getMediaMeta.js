...
async function refreshFromAniList(aniId, opts = {}) {
  // Routed through the global limiter + dedup + response cache.
  // Multiple SSRs hitting the same id within 60s collapse into one
  // upstream call. `skipCache` (audit) bypasses the Redis response cache +
  // limiter so a big audit fan-out doesn't spend Redis requests.
  ...
  if (media) {
    memSet(aniId, media);
    // Persist async — don't block the response on DB write. Skipped for audit
    // (skipCache) so the audit is purely read-only and never writes Turso/Redis.
    if (!opts.skipCache) upsertAnime(media).catch(() => {});
  }
  return media;
}
...
export async function getMediaMeta(aniId, opts = {}) {
  ...
  const promise = (async () => {
    // Audit (skipCache): skip the Redis-backed health check (it does a
    // redis.get) and just assume AniList is up — keeps the audit Redis-free.
    const anilistUp = opts.skipCache ? true : await isAnilistLikelyUp();

    if (anilistUp) {
      // 2a. AniList first — saves a Turso read on the happy path
      const fresh = await refreshFromAniList(aniId, opts);
    ...
    const fromDb = await readFromDb(aniId);
    if (fromDb) return fromDb;
    // DB empty & AniList down. This used to try AniList "one last time anyway"
    // on the theory that the health signal could be stale. It can't usefully:
    // anilistFetch now consults the same signal itself and returns null without
    // touching the network, so the call was pure latency. The signal refreshes
    // every 60s, which is the real bound on how stale this decision can be.
    return null;
  })();
