...
export async function aniListHomepageBatch() {
  ...
  try {
    ...
    const json = await anilistFetch({
      ...
      timeoutMs: 5000,
      label: "homepageBatch",
      // The home page keeps the result under its own key for longer than the
      // 30 min response cache: that SET (~60 KB) could never be read.
      cacheSuccess: false,
    });
    ...
    return {
      /* Ce lot est DEGRADE : il vient de Turso, pas d'AniList, et la moitie
         saison (heroPool / thisSeason / movies) est vide faute de repli fiable.
         Le drapeau existe pour une seule raison : l'appelant ne doit pas
         ECRIRE ce lot dans le cache Redis de deux heures. Sans lui, une panne
         AniList de trente secondes gelait une page d'accueil amputee pour deux
         heures, longtemps apres le retour du service. */
      degraded: true,
