...
const SERVERS = [
  ...
  // Anime-Sama Sendvid — re-enabled at user request. Historically slow to
  // seek/buffer, so it's ranked below Sibnet/Vidmoly (speed 4) — it shows up
  // last among the VF hosts, as a fallback when the faster ones miss the title.
  {
  ...
  // Ansembed is Vidmoly under a white-label domain (same embed page, same
  // …/hls2/…/master.m3u8 — see VIDMOLY_DOMAINS in lib/extractors.js), but
  // anime-sama lists it as a SEPARATE player with its own uploads: a title that
  // is missing or dead on the vidmoly.* chip is often present here. Same
  // browser-side extraction, no proxy, so it shares Vidmoly's speed rank.
  {
  ...
  // Anime-Sama Sendvid (VO) — re-enabled at user request; ranked below the
  // faster VOSTFR hosts (see the VF block above).
  {
  ...
  // Ansembed (VO) — Vidmoly white-label, separate uploads; see the VF block.
  {
