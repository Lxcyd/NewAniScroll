...
const SERVERS = [
  // ── Iframe embeds ──────────────────────────────────────────
  {
    id: "vidnest",
    name: "VidNest",
    // API: backend (extractVidnest) calls the new.vidnest.fun anime API, decodes
    // its custom-base64 payload, and returns a clean HLS m3u8 + multi-language
    // subtitle tracks — so it plays in OUR Vidstack player (custom chrome, subs,
    // skip overlay) instead of a foreign iframe. Keyed by AniList id; the
    // session's dub flag picks the sub/dub variant.
    type: "api",
    lang: "multi",
    source: "vidnest",
    speed: 1,
