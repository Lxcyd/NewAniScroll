...
const SERVERS = [
  // ── Iframe embeds ──────────────────────────────────────────
  {
    id: "vidnest",
    name: "VidNest",
    // Pure AniList-id iframe embed (https://vidnest.fun). No backend
    // extraction — buildSrc returns the URL directly and the player renders
    // it in the universal iframe chrome. The session's dub flag selects the
    // sub/dub variant (dub = English dub, matching our sub/dub toggle).
    // Listed under `multi` since the single entry adapts to whichever audio
    // the session is set to, exactly like the other iframe embed.
    type: "iframe",
    lang: "multi",
    speed: 1,
    buildSrc: ({ aniId, episode, dub }) =>
      `https://vidnest.fun/anime/${aniId}/${episode}/${dub ? "dub" : "sub"}`,
  },
  {
    id: "megaplay",
    name: "Megaplay",
